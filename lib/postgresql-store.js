...
module.exports = function (opts) {
  ...
  var camelToSnakeCache = {};
  var snakeToCamelCache = {};
  function camelToSnakeCase(field) {
    // replace "camelCase" with "camel_case"
    var cached = camelToSnakeCache[field];
    var result = '';
    if (cached) {
     // console.log('camelToSnakeCache:',cached);
      return cached;
    }
    else {
      upperCaseRegExp.lastIndex = 0 // just to be sure. does not seem necessay. String.replace seems to reset the regexp each time.

      result = field.replace(upperCaseRegExp, function (str, offset) {
        return ('_' + str.toLowerCase());
      });
      camelToSnakeCache[field] = result;
       //console.log('camelToSnakeCache Miss:', result);
      return result;
    }
  ...
  function snakeToCamelCase(column) {
    ...
    var cached = snakeToCamelCache[column];

    if (cached) {
       //console.log('snakeToCamelCache:',cached);
      return cached;
    }
    else{
     ...
     // console.log('snakeToCamelCache Miss:',field);
      snakeToCamelCache[column] = field;
      return field;
    }
